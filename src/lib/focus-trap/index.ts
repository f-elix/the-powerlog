...
if (typeof window !== 'undefined') {
	...
	const trapFocusListener = (event: KeyboardEvent) => {
		...
		const focusables: NodeListOf<HTMLElement> = parentNode.querySelectorAll(
	...
	window.addEventListener('keydown', trapFocusListener);
}

export const trapFocus = (node: HTMLElement): { destroy?: () => void } => {
